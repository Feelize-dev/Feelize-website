...
export const invokeLLM = async (req, res) => {
  try {
    ...
    // Pass whether we expect JSON so we can optimize the config
    const expectJSON = !!response_json_schema;
    const response = await generateWithLLM(finalPrompt, expectJSON);
  ...
  try {
    ...
    if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
      return res.status(503).json({
        success: false,
        message: "Email service not configured (missing env vars)",
      });
    }
...
export const uploadFile = (req, res) => {
  upload(req, res, function (err) {
    ...
    const fileUrl = `${req.protocol}://${req.get("host")}/data/${
      req.file.filename
    }`;
...
export const generateImage = async (req, res) => {
  ...
  res
    .status(501)
    .json({ success: false, message: "Image generation not implemented yet" });
};
