        ...
        <div className="flex flex-col lg:flex-row gap-8 relative z-10">
          ...
          <div className="lg:w-2/3">
            <div className="p-6 bg-slate-900/50 backdrop-blur-xl border border-cyan-500/20 rounded-2xl">
              <h2 className="text-2xl font-bold mb-6 text-cyan-400">Champion Roster</h2>
              <div className="space-y-6 max-h-[80vh] overflow-y-auto pr-2">
                {Object.entries(columns).map(([key, members]) => {
                  if (key === 'selectedTeam' || members.length === 0) return null;
                  return (
                    <div key={key}>
                      <h3 className="text-lg font-semibold capitalize mb-3 text-purple-400">{CATEGORY_STYLES[key]?.name || key}</h3>
                              ...
                              <Draggable key={member.id} draggableId={member.id} index={index}>
                                {(provided, snapshot) => (
                                  <div
                                    ...
                                    className={`transition-all duration-300 ${snapshot.isDragging ? 'scale-110 !opacity-100' : ''}`}>
          ...
          <div className="lg:w-1/3 flex flex-col gap-8">
            <div className="p-6 bg-slate-900/50 backdrop-blur-xl border border-purple-500/20 rounded-2xl flex-grow flex flex-col">
              <h2 className="text-2xl font-bold mb-6 text-purple-400">Your Project Team</h2>
              <Droppable droppableId="selectedTeam">
                {(provided, snapshot) => (
                  <div
                    ref={provided.innerRef}
                    {...provided.droppableProps}
                    className={`space-y-3 min-h-[300px] p-4 rounded-xl border-2 border-dashed transition-colors ${snapshot.isDraggingOver ? 'border-cyan-400 bg-cyan-500/10' : 'border-slate-700'}`}>
              ...
              <div className="flex gap-4 mt-6">
                <Link to={createPageUrl("StartProject")} className="w-full">
                  <Button disabled={columns.selectedTeam.length === 0} className="w-full bg-gradient-to-r from-cyan-400 to-blue-500 text-black font-bold">
                    Lock In Team
                  </Button>
                </Link>
...
const MemberCard = ({ member, category }) => {
  ...
  return (
    <div className={`relative w-32 h-44 rounded-xl overflow-hidden cursor-pointer group border-2 ${style.border} hover:shadow-2xl hover:shadow-cyan-500/30 transition-all duration-300 bg-slate-900`}>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center h-full p-2 text-center">
        <div className="w-16 h-16 rounded-full overflow-hidden border-2 border-white/20 group-hover:border-cyan-400/50 transition-colors mb-2">
          <img src={member.avatar.replace('size=256', 'size=128')} alt={member.name} className="w-full h-full object-cover" />
        </div>
        <h4 className="font-bold text-sm text-white leading-tight drop-shadow-md">{member.name}</h4>
        <p className="text-xs text-slate-300 leading-tight">{member.role}</p>
        <div className="mt-2">
          <member.icon className={`w-5 h-5 text-white/70 group-hover:text-cyan-300 transition-colors`} />
        </div>
      </div>
...
const LockedMemberCard = ({ member }) => (
  <div className="flex items-center gap-4 p-3 bg-slate-800/70 rounded-lg border border-slate-700 cursor-pointer hover:bg-slate-700/70 transition-colors">
    ...
    <motion.div
      ...
      <div className="w-2/3 p-8 overflow-y-auto">
        ...
        <div className="space-y-8">
          ...
          {/* New: Member Stats in Detail Panel */}
          {member.stats &&
            <div>
              <h4 className="text-sm font-bold uppercase tracking-widest text-cyan-400 mb-2">Base Stats</h4>
              <div className="grid grid-cols-2 gap-x-6 gap-y-3">
                <div className="flex justify-between items-center text-slate-300"><span>Engineering:</span> <span className="font-bold text-white">{member.stats.engineering}</span></div>
                <div className="flex justify-between items-center text-slate-300"><span>Product/Design:</span> <span className="font-bold text-white">{member.stats.product}</span></div>
                <div className="flex justify-between items-center text-slate-300"><span>Delivery/Process:</span> <span className="font-bold text-white">{member.stats.delivery}</span></div>
                <div className="flex justify-between items-center text-slate-300"><span>Strategy:</span> <span className="font-bold text-white">{member.stats.strategy}</span></div>
              </div>
            </div>
          }
        </div>
